...
*/
  ...
  /* to compute the ranking of the languages
   *     (`va0l langs`) by determining the number of Wikipedia articles that
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {rdd.cache()
  ...
  /* (3) `reduceByKey` so that the computation of the index and the ranking are combined.
